...
export default function HistoryView({ orders, onSelectOrder, onBack }: Props) {
    ...
    return (
        <div className="p-4 md:p-6 bg-white dark:bg-gray-800 rounded-lg shadow-xl animate-fade-in h-full min-h-[50vh] flex flex-col">
                        ...
                        {sortedOrders.map((order) => (
                            ...
                            >
                                <div className="flex justify-between items-start mb-2">
                                    ...
                                    <span className={`text-xs px-2.5 py-0.5 rounded-full font-bold uppercase ${order.status === 'completed' ? 'bg-green-100 text-green-700 dark:bg-green-900/50 dark:text-green-400' :
                                    ...
                                    <div className="text-sm text-gray-600 dark:text-gray-400">
                                        {order.assignment?.lawyer ? (
                                            <div className="flex items-center gap-2">
                                                <img
                                                    src={order.assignment.lawyer.avatarUrl || `https://ui-avatars.com/api/?name=${encodeURIComponent(order.assignment.lawyer.nome)}&background=4f46e5&color=fff`}
