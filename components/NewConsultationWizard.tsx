...
import type { ServiceTopic, ConsultationPackage, ConsultationType } from '../types';
...
type Step = 'topic' | 'package' | 'method' | 'confirm';

const steps: { id: Step; label: string; number: number }[] = [
  { id: 'topic', label: 'Tema', number: 1 },
  { id: 'package', label: 'Pacote', number: 2 },
  { id: 'method', label: 'Método', number: 3 },
...
export default function NewConsultationWizard({ onComplete, onBack }: Props) {
  ...
  const handleSelectTopic = (topic: ServiceTopic) => {
  ...
  const getStepStatus = (stepId: Step) => {
    const stepOrder: Step[] = ['topic', 'package', 'method', 'confirm'];
